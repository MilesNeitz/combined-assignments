...
public class Main {
	...
	public static List<Student> readStudents(File studentDirectory, JAXBContext jaxb) throws JAXBException {
		...
		for (File x : studentFiles) {
			
			Contact contact = (Contact) jaxbUnmarshaller.unmarshal(x);
			Student student = new Student();
			student.setContact(contact);
	...
	public static Session readSession(File rootDirectory, JAXBContext jaxb) throws JAXBException {
		...
		File dateDirectory = new File(rootDirectory.getPath() +  "\\" + (rootDirectory.list())[0]);
		
		session.setStartDate(dateDirectory.getName());

		File studentsDirectory = new File(dateDirectory.getAbsolutePath() + "\\" + (dateDirectory.list())[1]);

		session.setStudents(readStudents(studentsDirectory, jaxb));

		File instructorContactFile = new File(dateDirectory.getAbsolutePath() +  "\\" +  (dateDirectory.list())[0]);
